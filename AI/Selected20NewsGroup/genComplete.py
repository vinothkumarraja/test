...
def prepareClassifier(trainingFolder ,vectPath, tfidfPath, classifierPath):
	...
	joblib.dump(vectorizer, vectPath)
	joblib.dump(tfidf_transformer, tfidfPath)
	joblib.dump(classifier, classifierPath)
	return
...
def main():
	if(len(sys.argv) == 5):
		...
		prepareClassifier(trainingFolder, vectPath, tfidfPath, classifierPath)
	else:
		print("Format: python genComplete.py <trainingFolder> <file_name.vec> <file_name.tfidf> <classifierPath.clf>")
